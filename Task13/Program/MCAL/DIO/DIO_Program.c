...
u8 DIO_voidGetPinValue     (u8 copy_u8PORT ,u8 copy_u8PIN  ){
  ...
  if (copy_u8PORT <= PORTD && copy_u8PIN <= PIN7 ){
    u8 loc_u8Status =0;
switch(copy_u8PORT){
     case PORTA :  loc_u8Status = GET_BIT(PORTA_REG,copy_u8PIN );    break;
     case PORTB :  loc_u8Status = GET_BIT(PORTB_REG,copy_u8PIN );    break;
     case PORTC :  loc_u8Status = GET_BIT(PORTC_REG,copy_u8PIN );    break;
     case PORTD :  loc_u8Status = GET_BIT(PORTD_REG,copy_u8PIN );    break;
     }
    return loc_u8Status;
  }
}
